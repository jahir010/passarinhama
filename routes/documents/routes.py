...
#     mime_type:     str
#     file_size:     int
#     storage_path:  str   # object storage key — server-side only
...
# @router.post("/documents/upload-url", tags=["Documents"])
# async def get_upload_url(
#     body:         UploadUrlRequest,
#     current_user: User = Depends(get_current_user),
# ):
#     """
#     Return a presigned S3 upload URL for direct client-to-storage upload.
#     Client uploads directly to S3, then calls POST /documents with metadata.
#     Spec ref: §12.4
#     """
#     folder = await DocumentFolder.get_or_none(id=body.folder_id)
#     if not folder:
#         raise HTTPException(status_code=404, detail="Folder not found.")
#     await check_folder_access(folder, current_user, need_upload=True)

#     storage_key = f"documents/{folder.id}/{uuid.uuid4()}/{body.original_name}"

#     # ── Replace with real boto3/minio presigned URL ──
#     # presigned_url = s3_client.generate_presigned_url(
#     #     "put_object",
#     #     Params={"Bucket": BUCKET_NAME, "Key": storage_key, "ContentType": body.mime_type},
#     #     ExpiresIn=900,
#     # )
#     presigned_url = f"https://s3.example.com/{storage_key}?presigned=stub"
...
# ):
#     """
#     Register document metadata after the client has uploaded to S3.
#     FIX: document_count increment now uses F() for atomic DB update —
#     avoids race condition under concurrent uploads.
#     Spec ref: §12.4
#     """
#     folder = await DocumentFolder.get_or_none(id=body.folder_id)
#     if not folder:
...
#     # FIX: atomic increment — no race condition on concurrent uploads
#     await DocumentFolder.filter(id=folder.id).update(document_count=F("document_count") + 1)

#     await log_activity(
#         current_user, ActivityActionType.DOCUMENT_UPLOADED, "document", doc.id, body.original_name
#     )
...
# ):
#     """
#     Issue a short-lived presigned download URL (15 min).
#     storage_path is used internally but never returned to the client.
#     Spec ref: §12.4
#     """
#     doc = await Document.get_or_none(id=document_id)
#     if not doc:
...
#     # presigned_url = s3_client.generate_presigned_url(
#     #     "get_object",
#     #     Params={"Bucket": BUCKET_NAME, "Key": doc.storage_path},
#     #     ExpiresIn=900,
#     # )
#     presigned_url = f"https://s3.example.com/{doc.storage_path}?presigned=stub&expires=900"
...
#     # FIX: atomic decrement — no race condition, no negative values
